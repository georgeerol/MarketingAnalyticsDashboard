/**
 * Hook for fetching MMM data from the API
 */
...
/**
 * Hook for fetching MMM data
 *
 * Provides methods like getChannelSummary(), getResponseCurves(), etc.
 * Handles auth tokens and loading states.
 */
export function useMMMData() {
  ...
  const getContribution = async (
    ...
    | MMMContribution
    | { channels: string[]; data: any[]; summary: Record<string, any> }
  > => {
    setLoading(true);
    setError(null);
    try {
      const url = channel
        ? `/mmm/contribution?channel=${channel}`
        : "/mmm/contribution";
  ...
  const getResponseCurves = async (
    channel?: string,
  ): Promise<MMMResponseCurve | { curves: Record<string, any> }> => {
    setLoading(true);
    setError(null);
    try {
      const url = channel
        ? `/mmm/response-curves?channel=${channel}`
        : "/mmm/response-curves";
      const data = await fetchWithAuth(url);
      return data;
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to fetch response curves",
      );
      throw err;
    } finally {
      setLoading(false);
  ...
  const getChannelSummary = async (): Promise<Record<string, MMMChannel>> => {
    ...
    try {
      ...
      const [contributionData, responseCurvesData] = await Promise.all([
        fetchWithAuth("/mmm/contribution") as Promise<{
          summary: Record<string, any>;
        }>,
        fetchWithAuth("/mmm/response-curves") as Promise<{
      ...
      channels.forEach((channel) => {
        ...
        if (channelData && curveData) {
          summary[channel] = {
            ...
            contribution_share:
              channelData.total /
              Object.values(contributionData.summary).reduce(
                (sum: number, ch: any) => sum + ch.total,
                0,
              ),
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch channel summary",
      );
      throw err;
    } finally {
      setLoading(false);
