...
import { SimpleContributionChart, SimpleMMInsights } from '@/components/mmm/simple-charts'
...
describe('Enhanced MMM Components', () => {
  ...
  describe('SimpleContributionChart with Performance Indicators', () => {
    ...
    it('renders enhanced contribution chart with performance indicators', async () => {
      ...
      const mockResponseCurves = {
        curves: {
          'Channel0': { efficiency: 1.58, saturation_point: 30000, adstock_rate: 0.77 },
          'Channel1': { efficiency: 0.92, saturation_point: 35000, adstock_rate: 0.64 },
          'Channel2': { efficiency: 1.43, saturation_point: 40000, adstock_rate: 0.40 }
        }
      }

      const mockGetChannelSummary = vi.fn().mockResolvedValue(mockChannelSummary)
      const mockGetResponseCurves = vi.fn().mockResolvedValue(mockResponseCurves)
      ...
      await waitFor(() => {
        expect(screen.getByText('Channel Performance & Efficiency')).toBeInTheDocument()
      })
    ...
    it('displays performance indicators correctly', async () => {
      ...
      const mockResponseCurves = {
        curves: {
          'Channel0': { efficiency: 1.58, saturation_point: 30000, adstock_rate: 0.77 }, // High performer
          'Channel1': { efficiency: 0.75, saturation_point: 35000, adstock_rate: 0.64 }  // Underperformer
        }
      }
  ...
  describe('SimpleMMInsights with Enhanced Recommendations', () => {
    ...
    it('generates enhanced insights with real data', async () => {
      ...
      const mockResponseCurves = {
        curves: {
          'Channel0': { efficiency: 1.58, saturation_point: 30000, adstock_rate: 0.77 },
          'Channel1': { efficiency: 0.75, saturation_point: 35000, adstock_rate: 0.64 }
        }
      }
    ...
    it('generates budget optimization recommendations', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Shifting budget from.*Channel1.*to.*Channel0/)).toBeInTheDocument()
      })
    })
    ...
    it('handles saturation alerts', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Channel0.*low saturation/)).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Component Integration', () => {
    ...
    it('handles API errors gracefully', async () => {
      const mockGetChannelSummary = vi.fn().mockRejectedValue(new Error('API Error'))
      const mockGetResponseCurves = vi.fn().mockRejectedValue(new Error('API Error'))
